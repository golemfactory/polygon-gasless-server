...
import { validateTransferMetaTxArguments } from "../validators.ts";
...
Deno.test({
  ...
  async fn() {
    ...
    let error_details = await validateTransferMetaTxArguments(
      sender,
      args,
      "latest"
    );
    ...
    error_details = await validateTransferMetaTxArguments(
      "4DCeBf483fA7f31FfCee6e4EAffC1D78308Ec2cD",
      args,
      "latest"
    );
    assertEquals(error_details, "Sender and recipient addresses must differ");
    ...
    error_details = await validateTransferMetaTxArguments(
      "4DCeBf483fA7f31FfCee6e4EAffC1D78308Ec2cD".toUpperCase(),
      args,
      "latest"
    );
    assertEquals(error_details, "Sender and recipient addresses must differ");
...
Deno.test({
  ...
  async fn() {
    const block_number =
      "0xaa4676e812653c037ec88233b8a405a6679ac12222f01744b63996c098438208";
    const args: TransferArgs = {
      recipient: "0x4DCeBf483fA7f31FfCee6e4EAffC1D78308Ec2cD",
      amount: "1",
    };
    ...
    assertEquals(
      error_details,
      "Provided block is too old and can contain stale data"
    );
  },
});
...
Deno.test({
  ...
  async fn() {
    ...
    const sender = "0xFeaED3f817169C012D040F05C6c52bCE5740Fc37";
    const block_hash = await web3.eth
      .getBlock("latest")
